...
`,pt=["monday","tuesday","wednesday","thursday","friday","saturday","sunday"],ut=["monday","tuesday","wednesday","thursday","friday"],ft={monday:"Hétfő",tuesday:"Kedd",wednesday:"Szerda",thursday:"Csütörtök",friday:"Péntek",saturday:"Szombat",sunday:"Vasárnap"},yt=["sunday","monday","tuesday","wednesday","thursday","friday","saturday"],mt=/^([01]\d|2[0-3]):[0-5]\d$/;function gt(t){const[e,s]=t.split(":").map(Number);return 60*e+s}function $t(t){return pt.includes(t)}function bt(t,e){if(null==t||""===t)return{subject:null};if("string"==typeof t)return{subject:t};if("object"==typeof t){if("string"!=typeof t.subject||0===t.subject.length)throw new Error(`${e}: a \`subject\` mező kötelező és nem üres szöveg.`);if(void 0!==t.time&&"string"!=typeof t.time)throw new Error(`${e}: a \`time\` mező szöveg kell hogy legyen.`);return{subject:t.subject,time:t.time}}throw new Error(`${e}: érvénytelen cella (sem string, sem objektum, sem null).`)}const _t="school-timetable-card";let vt=class extends nt{constructor(){super(...arguments),this._todayKey=yt[(new Date).getDay()]}setConfig(t){this._config=function(t){if(!t||"object"!=typeof t)throw new Error("Érvénytelen konfiguráció: üres vagy nem objektum.");const e=t;if(!Array.isArray(e.periods)||0===e.periods.length)throw new Error("`periods` mező kötelező, és legalább egy órát kell tartalmaznia.");const s=e.periods.map((t,e)=>{if(!t||"object"!=typeof t)throw new Error(`periods[${e}] nem érvényes objektum.`);const{start:s,end:r,label:i}=t,o=void 0!==s;if(o!==(void 0!==r))throw new Error(`periods[${e}]: a \`start\` és \`end\` mezők együtt megadandók vagy együtt elhagyandók.`);if(o){if("string"!=typeof s||!mt.test(s))throw new Error(`periods[${e}].start érvénytelen (HH:MM formátum kell, pl. "08:00").`);if("string"!=typeof r||!mt.test(r))throw new Error(`periods[${e}].end érvénytelen (HH:MM formátum kell).`);if(gt(s)>=gt(r))throw new Error(`periods[${e}]: a kezdő időpont (${s}) nem lehet későbbi a végénél (${r}).`)}return{label:"string"==typeof i?i:"",...o?{start:s,end:r}:{}}}),r=Array.isArray(e.days)?e.days.map((t,e)=>{if("string"!=typeof t||!$t(t))throw new Error(`days[${e}] ismeretlen nap: "${String(t)}". Megengedett: ${pt.join(", ")}.`);return t}):[...ut];if(!e.schedule||"object"!=typeof e.schedule)throw new Error("`schedule` mező kötelező.");const i={};for(const[t,r]of Object.entries(e.schedule)){if(!$t(t))throw new Error(`schedule: ismeretlen nap "${t}". Megengedett: ${pt.join(", ")}.`);if(!Array.isArray(r))throw new Error(`schedule.${t} listának kell lennie.`);if(r.length>s.length)throw new Error(`schedule.${t} hossza (${r.length}) nagyobb, mint a periods hossza (${s.length}).`);const e=r.map((e,s)=>bt(e,`schedule.${t}[${s}]`));for(;e.length<s.length;)e.push({subject:null});i[t]=e}if(void 0!==e.show_times&&"boolean"!=typeof e.show_times)throw new Error("`show_times` mezőnek boolean-nak kell lennie (true / false).");return{type:"string"==typeof e.type?e.type:"custom:school-timetable-card",title:"string"==typeof e.title?e.title:void 0,days:r,showTimes:!1!==e.show_times,periods:s,subjects:e.subjects&&"object"==typeof e.subjects?e.subjects:{},schedule:i}}(t)}getCardSize(){return(this._config?.periods.length??0)+1}connectedCallback(){super.connectedCallback(),this._updateToday(),this._refreshTimer=window.setInterval(()=>this._updateToday(),3e5)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._refreshTimer&&(window.clearInterval(this._refreshTimer),this._refreshTimer=void 0)}_updateToday(){const t=yt[(new Date).getDay()];t!==this._todayKey&&(this._todayKey=t)}render(){if(!this._config)return B``;const t=this._config,e=t.days.includes(this._todayKey),s=t.showTimes&&t.periods.some(t=>void 0!==t.start&&void 0!==t.end),r=s?"":"no-times";return B`
      <ha-card>
    ...
    `}_renderRow(t,e,s){const r=this._config,i=r.showTimes&&void 0!==t.start&&void 0!==t.end;return B`
      <tr>
        <td class="time-col">
          ${t.label?B`<span class="period-label">${t.label}</span>`:q}
          ${i?B`<span class="period-time">${t.start}–${t.end}</span>`:q}
        </td>
        ${r.days.map(t=>{const i=s&&t===this._todayKey,o=r.schedule[t]?.[e]??{subject:null};return B`<td class=${i?"today":""}>${this._renderLesson(o)}</td>`})}
      </tr>
